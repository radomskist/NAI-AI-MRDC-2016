...
void ObjectSegmentor<WIDTH, HEIGHT>::findObjects() {
	...
	this->queue.reserve(WIDTH * HEIGHT * 0.25);
	this->buckets.reserve(WIDTH * HEIGHT);
	...
	while (!this->checkedStatus.all()) {
		if (this->queue.size() == 0) {
			...
			if (this->buckets[currentBucket].points.size() < (WIDTH * HEIGHT * 0.0005)) {
				this->buckets[currentBucket].points.clear(); // Delete the current bucket as it's too small
			}
			else {
				// Create and add a new bucket
				ObjectBucket bucket;
	...
	std::cout << "Buckets: " << this->buckets.size() << "\t";
	std::cout << "Current Bucket: " << currentBucket << "\t";
	std::cout << "Queue Size: " << this->queue.size() << "\t";
	std::cout << "Pixels Checked: " << this->checkedStatus.count() << std::endl;
}
...
cv::Point3f ObjectSegmentor<WIDTH, HEIGHT>::getFirstUncheckedPoint() {
	...
	for (int i = 0; i < this->checkedStatus.size(); i++) {
		if (this->checkedStatus[i] == 0) {
			...
			//if (this->getColorData(this->getPoint(x, y))->a > 0) {
