...
void applyGaussianBlur(GLubyte *bytes) {
	...
	cv::Mat foundPixels(HEIGHT, WIDTH, CV_8U);
	...
	cv::cvtColor(dest, foundPixels, CV_BGR2HSV);
	cv::inRange(foundPixels,
		//cv::Scalar(55, 99, 47),
		//cv::Scalar(80, 196, 177),
		//cv::Scalar(70, 10, 35),
		//cv::Scalar(150, 200, 200),
		cv::Scalar(30, 95, 47),
		cv::Scalar(79, 196, 220),
		foundPixels);
	...
	std::cout << foundPixels.size() << std::endl;
	std::cout << foundPixels.total() << std::endl;
	std::cout << mat.total() << std::endl;
	for (int i = 0; i < HEIGHT; i++) {
		//cv::Vec3b* isValid = foundPixels.ptr<cv::Vec3b>(i); // point to first pixel in row
		//cv::Vec4b* pixel = mat.ptr<cv::Vec4b>(i); // point to first pixel in row
		for (int j = 0; j < WIDTH; j++) {
			//std::cout << i << "|" << j << "\t" << (int)foundPixels.data[((WIDTH * i) + j)] << std::endl;
			//if (isValid[j][0] == 0) {
			if (foundPixels.data[(WIDTH * i) + j] == 0) {
				bytes[((WIDTH * i) + j) * 4 + 0] = 0;
				bytes[((WIDTH * i) + j) * 4 + 1] = 0;
				bytes[((WIDTH * i) + j) * 4 + 2] = 0;
				bytes[((WIDTH * i) + j) * 4 + 3] = 0;
			}
		}
	}
